...
const AddRegion = () => {
  const { RegionModel, CityID } = useSelector((state) => state.portals);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      `${process.env.NEXT_PUBLIC_API_URL}/api/cities/${CityID}/regions/`,
        formData
      );
      console.log("SubPortal created:", response.data);
      dispatch(openRegionModal({ open: false }));
  ...
  return (
    <div>
      <Modal
        ...
        activeModal={RegionModel}
        onClose={() => dispatch(openRegionModal({ open: false }))}
      >
